...
def import_datasets(loc):
    ...
    train_data = pd.read_pickle('data_EU\EU' + loc + '_train_data.pkl')
    val_data = pd.read_pickle('data_EU\EU' + loc + '_test_data.pkl')
    test_data = pd.read_pickle('data_EU\EU' + loc + '_val_data.pkl')

    return train_data, val_data, test_data

# loc is which part of Europe you want to analyze, by choosing loc[0], you choose all regions together
loc = ['', '_north_scandinavia', '_south_scandinavia', '_central_europe']
train_data, val_data, test_data = import_datasets(loc[0])
...
plt.plot([-3000, 2000], [-800, -400], '-k')
plt.title('Plot for EU' + loc[0])
plt.xlabel('data')
plt.ylabel('model')
plt.show()